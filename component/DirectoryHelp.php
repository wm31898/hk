...
class DirectoryHelp {
	 ...
	 * @return bool
	 */
	public static function create_dir_with_filepath($path,$mode=0777){
		return self::create_dir(dirname($path),$mode);
	}
	...
	public static function all_copy($source, $destination,$justnew = false){
		if(!is_dir($source)){
			if(!file_exists(dirname($destination))){
				DirectoryHelp::create_dir(dirname($destination));
			}
	...
	{
		self::create_dir_with_filepath($path);
